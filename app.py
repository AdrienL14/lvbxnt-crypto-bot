...
# ✅ Comme demandé : les 10 déjà cochées
...
def format_signal(sig):
    ...
    if sig["direction"] == "NO TRADE":
        return (
            f"👑 *LVBXNT CRYPTO BOT — V2.6.2 FOREX LOGIC CLONE* 👑\n\n"
            f"💰 *Crypto:* `{sig['symbol']}`\n"
            f"⚠️ *Signal:* *NO TRADE*\n"
...
@app.route("/")
def home():
    return "LVBXNT CRYPTO BOT V2.6.2 FOREX LOGIC CLONE is running!"
...
def telegram_webhook():
    ...
    if "message" in data:
        ...
        if normalized == "/start" or raw_text.lower() == "/start":
            welcome = (
                "👑 *LVBXNT CRYPTO BOT — V2.6.2 FOREX LOGIC CLONE* 👑\n\n"
